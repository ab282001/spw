...
const category = (props) => {
    ...
    const handleClick=async(productRangeCategory)=>{
        let data=await fetch(`${config.host}/api/getAllProducts`)
                ...
                return<div key={product.product_id} className="col-md-4 my-3">
                    ...
                    <div id={`${product.product_id}`} className="card product-card">
                        <div className="img-thumbnail img-fluid mx-5 mt-4 mb-1 p-5">
                            <Image width={500} height={500} src={`/ProductsImages/${product.product_image}`}></Image>
                        </div>
                    <div className="card-body m-auto">
                    <Link href={`/display-product/${product.product_id}?product=${product.product_name}`}><a className="card-title text-dark text-decoration-none text-center"><h5>{product.product_name}</h5></a></Link>
...
export async function getServerSideProps(context){
    ...
    let allProducts=[]
    allProductsArray.map((product)=>{
        if(context.query.category===product.product_category){
            allProducts.push(product)
        }
    })
    return{
